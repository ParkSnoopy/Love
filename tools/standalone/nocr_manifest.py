#!/usr/bin/env python3
"""nocr_manifest.py - Update or create manifest.json from nocr .sqlite files

Scans a directory of .sqlite files, reads metadata from the version table,
and builds/updates manifest.json entries.

Usage:
    python nocr_manifest.py db/                              # scan recursively, write ./manifest.json
    python nocr_manifest.py db/ -o manifest.json             # specify output path
    python nocr_manifest.py db/ --type commentary            # set type field (override auto-detect)
    python nocr_manifest.py db/ --source nocr                # source label (override auto-detect)
    python nocr_manifest.py db/ --rel-dir subdir             # file prefix override (not needed with dist/ layout)
    python nocr_manifest.py db/ --manifest existing.json     # merge with existing
...
def main():
    ap = argparse.ArgumentParser(
        description="Update or create manifest.json from nocr .sqlite files.",
    ...
    ap.add_argument("--source", default="nocr", help="Source label (default: nocr)")
    ap.add_argument(
        "--rel-dir",
        default="",
