#!/usr/bin/env python3
"""helloao_manifest.py - Update or create manifest.json from helloao .sqlite files

Scans a directory of .sqlite files, reads metadata from the version table,
and builds/updates manifest.json entries.

Usage:
    python helloao_manifest.py db/                              # scan recursively, write ./manifest.json
    python helloao_manifest.py db/ -o manifest.json             # specify output path
    python helloao_manifest.py db/ --type commentary            # set type field (override auto-detect)
    python helloao_manifest.py db/ --source helloao             # source label (override auto-detect)
    python helloao_manifest.py db/ --rel-dir subdir             # file prefix override (not needed with dist/ layout)
    python helloao_manifest.py db/ --manifest existing.json     # merge with existing
...
# ISO 639-3 -> English name (common helloao codes)
LANG_NAMES = {
    "aar": "Afar",
    "afr": "Afrikaans",
    "aka": "Akan",
...
def read_version_table(db_path: Path) -> dict | None:
    """Read slug and label from a helloao .sqlite version table."""
    try:
...
def build_entry(
    db_path: Path, scan_dir: Path, rel_dir: str, entry_type: str, source: str
) -> dict | None:
...
def pack_sort_key(entry: dict) -> tuple:
    type_rank = {"bible": 0, "commentary": 1}.get(entry["type"], 2)
    korean_rank = 0 if entry["language"] == "Korean" else 1
    return (
        type_rank,
        korean_rank,
...
def main():
    ap = argparse.ArgumentParser(
        description="Update or create manifest.json from helloao .sqlite files.",
    ...
    ap.add_argument(
        "--type",
        default="bible",
        choices=["bible", "commentary"],
        help="Entry type (default: bible)",
    )
    ap.add_argument(
        "--source", default="helloao", help="Source label (default: helloao)"
    )
    ap.add_argument(
        "--rel-dir",
        default="",
    ...
    if manifest_path.exists():
        try:
            ...
            print(
                f"Loaded {len(existing)} existing entries from {manifest_path}",
                flush=True,
            )
    ...
    merged = sorted(existing.values(), key=pack_sort_key)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(merged, f, indent=2, ensure_ascii=False)
        f.write("\n")

    print(
        f"\nDone: {added} added, {updated} updated, {skipped} skipped -> {out_path}",
        flush=True,
    )
