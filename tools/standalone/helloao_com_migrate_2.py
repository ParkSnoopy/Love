#!/usr/bin/env python3
"""helloao_com_migrate_2.py - Convert commentary XLSX (from migrate_1) to SQLite

Commentaries use the same 3-sheet format as Bibles but book_id=0 (INTRO)
uses testament='INTRO' instead of 'OT'/'NT'.

Usage:
    python helloao_com_migrate_2.py matthew-henry.xlsx
    python helloao_com_migrate_2.py xlsx/ --output-dir ./db
    python helloao_com_migrate_2.py xlsx/ --single-db commentaries.db
"""
...
def create_tables(conn):
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS books (
            ...
            name TEXT NOT NULL,
            testament TEXT NOT NULL CHECK(testament IN ('OT', 'NT', 'INTRO')),
...
def import_xlsx(xlsx_path: Path, conn) -> int:
    ...
    for row in ws2.iter_rows(min_row=2, values_only=True):
        ...
        if not text:
            continue  # skip empty/null verse rows
...
def convert_xlsx_to_sqlite(
    xlsx_path: Path, output_dir: Path, single_conn=None
):
    slug = parse_slug_from_name(xlsx_path)
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert commentary XLSX file(s) to SQLite database.",
    ...
    ap.add_argument(
        ...
        "-o",
        default="./db",
        help="Output directory for .sqlite files (default: ./db)",
    )
