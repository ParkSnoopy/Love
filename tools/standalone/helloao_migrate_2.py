#!/usr/bin/env python3
"""helloao_migrate_2.py - Convert helloao XLSX (from migrate_1) to SQLite

Usage:
    python helloao_migrate_2.py eng_BSB.xlsx
    python helloao_migrate_2.py xlsx/ --output-dir ./db
    python helloao_migrate_2.py xlsx/ --single-db bibles.db
"""
...
def parse_slug_from_name(xlsx_path: Path) -> str:
    """Derive slug from filename: eng_BSB.xlsx -> BSB, or fallback to stem."""
    stem = xlsx_path.stem
...
def convert_xlsx_to_sqlite(
    xlsx_path: Path, output_dir: Path, single_conn=None, single_meta=None
):
    slug = parse_slug_from_name(xlsx_path)
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert helloao XLSX file(s) to SQLite database.",
    ...
    ap.add_argument(
        ...
        "-o",
        default="./db",
        help="Output directory for .db files (default: ./db)",
    )
