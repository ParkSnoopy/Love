...
- `application theme`: the settings theme action opens the Custom editor directly. Custom stores one explicit Light or Dark base plus one opaque primary-color ID selected from the application palette; Apply persists both values together, while Cancel leaves the active theme unchanged. Primary foreground/container roles are contrast-derived. No preset or System themes are exposed.
