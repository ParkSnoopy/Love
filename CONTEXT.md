...
- `application themes`: theme selection cycles through System, Light Orange, Light Green, Dark Orange, and Dark Purple. Light Green uses `#228B22` (forest green) as its primary, and Dark Orange uses Light Orange's `#CC785C` primary on a dark surface palette. Generic Light and Dark themes are not exposed.
